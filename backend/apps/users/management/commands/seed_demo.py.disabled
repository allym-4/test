...
# ── Full timetable ────────────────────────────────────────────────────────────
# (name, level_tag, instructor_first, room_key, day_of_week 0=Mon,
#  start_time, duration_mins, session_type, capacity)
# room_key: 'rhapsody'=yellow, 'box'=purple, 'closet'=Janitor's Closet
TIMETABLE = [
    # ── Monday ───────────────────────────────────────────────────────────────
...
# ── Demo enrolments ───────────────────────────────────────────────────────────
# Each entry: (student_slug, session_name, instructor_first, day_of_week)
# This uniquely identifies a session row in the timetable above.
DEMO_ENROLMENTS = [
    # jess — Level 2 Monday with Chloe (intermediate, 1 class)
    ('jess',   'Level 2',      'Chloe',    0),
    # tara — Level 2 Monday + Invert Tech Wednesday (2 classes)
    ('tara',   'Level 2',      'Chloe',    0),
    ('tara',   'Invert Tech',  'Maz',      2),
    # dana — Level 3 Monday (advanced, 1 class)
    ('dana',   'Level 3',      'Chloe',    0),
    # nina — Level 1 Wednesday + Strip Virgin Thursday (2 classes)
    ('nina',   'Level 1',      'Chloe',    2),
    ('nina',   'Strip Virgin', 'Amy',      3),
    # sophie — Level 4 Monday + Dance Tech Monday (same night, 2 classes)
    ('sophie', 'Level 4',      'Chloe',    0),
    ('sophie', 'Dance Tech',   'Chloe',    0),
    # alex — Level 1 Tuesday (beginner, 1 class)
    ('alex',   'Level 2',      'Avalon',   1),
    # riley — Level 5 Tuesday + 3 Tricks Tuesday (same night, 2 classes)
    ('riley',  'Level 5',      'Chloe',    1),
    ('riley',  '3 Tricks',     'Avalon',   1),
    # morgan — Strip Tuesday + Floor Virgin Monday (specialty focus, 2 classes)
    ('morgan', 'Strip',        'Mimi',     1),
...
class Command(BaseCommand):
    ...
    def _seed(self):
        ...
        # Silence signals — no welcome emails, no fee triggers
        ...
        session_index = {}  # (name, instructor_first, day) -> ClassSession
        ...
        occ_by_session = {}  # session_id -> {'past': [], 'upcoming': []}
        ...
        # nina and alex are first-timers who did a trial before enrolling
        first_timers = {'nina', 'alex'}

        enrolment_map = {}  # slug -> [Enrolment]
        ...
        absence_slots = {
            'tara':  0,  # absent from first past occurrence of first enrolled class
            'nina':  1,  # absent from second past occurrence
            'alex':  0,
        }
        for slug, enrolment_list in enrolment_map.items():
            for enrolment, sess in enrolment_list:
                ...
                # Apply absence for first enrolled class only
                ...
                if absent_idx is not None and absent_idx < len(past_occs):
                    ...
                    absence_slots.pop(slug)  # only apply once per student

        # ── Payments ─────────────────────────────────────────────────────────
        self.stdout.write('Creating payments...')
        admin_user = User.objects.filter(role='admin').first()

        # Fix trial price in StudioSettings if it doesn't match the live price
        try:
            ...
            if float(settings.price_trial or 0) != PRICE_TRIAL:
                ...
                self.stdout.write(f'  Updated trial price to ${PRICE_TRIAL}')
        except Exception:
            pass
        ...
        for slug, enrolment_list in enrolment_map.items():
            ...
            if not Payment.objects.filter(student=student, description__contains='Season 3').exists():
                # First-timers: trial charge first, then season fee minus trial credit
                if slug in first_timers:
                    ...
                    Payment.objects.create(
                        ...
                        amount=PRICE_TRIAL,
                        description='Trial payment received',
                        created_by=admin_user,
                    )
                    # Season fee with trial deducted
                    ...
                    Payment.objects.create(
                        ...
                        payment_type='payment',
                        amount=season_amount,
                        description='Season payment received',
                        created_by=admin_user,
                    )
                else:
                    Payment.objects.create(
                        ...
                        payment_type='charge',
                        amount=amount,
                        description=f'Season 3 — {class_names}',
                        created_by=admin_user,
                    )
                    # Most students paid in full; jess has a balance owing
        ...
        if Notification:
            ...
            Notification.objects.get_or_create(
                ...
                defaults=dict(
                    body='A catch-up credit has been added for your recent absence — use it to book any casual class.',
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f'\n  To remove demo data:\n'
            f'    python manage.py seed_demo --clear\n'
        ))
    ...
    def _resolve_instructors(self):
        """Return dict of first_name_lower -> User for all instructors in the timetable."""
        from apps.users.models import User
    ...
    def _dates_in_range(self, weekday, start, end):
        """Return all dates with the given weekday (0=Mon) in [start, end]."""

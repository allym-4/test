...
export default function EditStudentModal({ student, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    // Always send at minimum an empty patch so server can validate
    try {
      const { data } = await client.patch(`/api/users/${student.id}/`, changed)
      onSaved(data)
      onClose()
    } catch (err) {
      const detail = err.response?.data
      setError(detail ? JSON.stringify(detail) : 'Failed to save changes.')
    } finally {
      setSaving(false)
    }
  }

...
export default function AdminTimetable() {
  ...
  const byDay = DAYS.map((day, i) => ({
    day,
    dayFull: DAYS_FULL[i],
    sessions: sorted.filter(s => s.day_of_week === i),
  }))
    ...
    <div>
      <div className="page-header" style={{ flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-lime btn-sm">+ Add Class</button>
        </div>
      </div>
        ...
        <div className="tbl-section">
          <table>
            ...
            <tbody>
              {sorted.map(s => {
                ...
                return (
                  <tr key={s.id} className="clickable">
                    ...
                    <td style={{ color: isToday || isPast ? 'var(--lime)' : 'var(--grey)', fontFamily: "'Archivo Black', sans-serif", fontSize: 14 }}>
              ...
              {sorted.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--grey)', padding: '32px 0' }}>No classes found</td></tr>
              )}
            </tbody>
          </table>
        </div>
